...
* **🧩 멀티 스도쿠 (Sudoku):** 난이도 설정, 협동 턴제 모드 및 실시간 관전 가능한 스피드 레이스 경쟁 모드, Solo 모드 지원.
  - **스피드 레이스 시각성 개선:** 상대방과 자신의 남은 칸 대비 해결 비율(%) 및 개수를 실시간 프로그레스 바로 볼 수 있어 경쟁 상황을 한눈에 체감할 수 있습니다.
  - **상대방 정답 번호 구분:** 내 보드의 입력 값은 파란색(#2563eb)으로 표시되며, 실시간으로 관전하는 상대방 보드판의 입력 값은 눈에 띄는 적색(#dc2626)으로 다르게 표시되어 상대방의 퍼즐 해결 현황을 훨씬 더 직관적으로 비교할 수 있습니다.
  - **극적인 승패 연출 (Option 1):** 게임 종료 시 화면에 글래스모피즘 결과 레이어가 서서히 뜨며, 승리 시에는 화려한 색종이(Confetti) 폭죽이 터지고, 패배 시에는 보드판이 슬프게 흔들리는 화면 진동(Screen Shake) 효과가 3초간 지속된 후 결과창으로 부드럽게 전환됩니다.
  - **상세 통계 및 점수 대시보드:** 결과 화면에 소요 시간, 실수 횟수, 입력 정확도(%) 및 난이도별 소요 시간에 따른 보너스와 실수 감점이 반영된 최종 게임 점수(점수판)를 시각화하여 재미 요소를 한층 높였습니다.
